...
<div class="content-card p-3">
  ...
  <form method="POST" action="{{ $position->exists ? route('admin.positions.update', $position->id) : route('admin.positions.store') }}">@csrf @if($position->exists) @method('PUT') @endif
    <div class="form-row">
      ...
      <div class="form-group col-md-6"><label>Lokasi</label><input name="location" class="form-control" value="{{ old('location', $position->location) }}" required placeholder="cth: Surabaya"></div>
    </div>
